...
static float cameraPointLength( demoCameraPoint_t *point ) {
	...
	cameraPointMatch( (const demoCameraPoint_t *)point, CAM_ORIGIN, (const demoCameraPoint_t **)match );
	...
	posGet( 0.0f, demo.camera.smoothPos, (const vec3_t *)control, lastOrigin );
	while (step < 1) {
		addStep = 1 - step;
		if (addStep > 0.01f)
			addStep = 0.01f;
...
static qboolean cameraOriginAt( int time, float timeFraction, vec3_t origin ) {
	...
	if (!match[1]) {
		...
		if (!match[3])
			return qfalse;
		cameraPointMatch( (const demoCameraPoint_t *)match[2], CAM_ORIGIN, (const demoCameraPoint_t **)match );
		searchLen = 0;
	} else if (!match[2]) {
	...
	posGet( 0.0f, demo.camera.smoothPos, (const vec3_t *)control, origin );
	while (step < 1) {
		addStep = 1 - step;
		if (addStep > 0.01f)
			addStep = 0.01f;
...
static float cameraPointAnglesLength( demoCameraPoint_t *point ) {
	...
	cameraPointMatch( (const demoCameraPoint_t *)point, CAM_ANGLES, (const demoCameraPoint_t **)match );
