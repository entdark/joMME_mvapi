...
extern void trap_MVAPI_FX_Reset ( void );
extern void trap_MVAPI_MME_Capture( const char *baseName, float fps, float focus, float radius );
extern int trap_MVAPI_MME_SeekTime( int seekTime );
extern void trap_MVAPI_MME_Music( const char *musicName, float time, float length );
extern void trap_MVAPI_MME_TimeFraction( float timeFraction );
extern void trap_MVAPI_RandomSeed( int time, float timeFraction );
extern void trap_MVAPI_S_UpdateScale( float scale );
...
static void demoSynchMusic( int start, float length ) {
	...
	trap_MVAPI_MME_Music( mov_musicFile.string , start * 0.001f, length );
}
...
static void VibrateView( const float range, const int eventTime, const float fxTime, const float wc, float scale, vec3_t origin, vec3_t angles) {
	...
	origin[2] += shadingTime * shadingTime * sin(shadingTime * M_PI * 23.0) * scale;
	angles[ROLL] += shadingTime * shadingTime * sin(shadingTime * M_PI * 1.642) * scale * 0.7f * sign;
}
...
//		trap_MME_BlurInfo( &blurTotal, &blurIndex );
		blurTotal = CG_Cvar_Get("mme_blurFrames");
		blurIndex = 0;
	...
	cg.time = trap_MVAPI_MME_SeekTime( demo.line.time );
	...
	trap_MVAPI_RandomSeed(cg.time % 10000000, cg.timeFraction * 1000);

	//silly hack :s
	if (demo.play.paused || !frameSpeed) {
		static float lastTimeFraction = 0.0f;
		if (!frameSpeed) {
			trap_FX_AdjustTime(cg.time, lastTimeFraction, cg.frametime, cg.refdef.vieworg, cg.refdef.viewaxis);
		} else {
			trap_FX_AdjustTime(cg.time, 0.0f, cg.frametime, cg.refdef.vieworg, cg.refdef.viewaxis);
			lastTimeFraction = cg.timeFraction;
		}
	} else {
		trap_FX_AdjustTime(cg.time, cg.timeFraction, cg.frametime, cg.refdef.vieworg, cg.refdef.viewaxis);
	}
	...
	trap_MVAPI_S_UpdateScale( frameSpeed );
	...
	if (captureFrame) {
		...
		trap_MVAPI_MME_Capture( fileName, captureFPS, demo.viewFocus, demo.viewRadius );
	} else {
